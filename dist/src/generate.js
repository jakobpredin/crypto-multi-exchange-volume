...
let appendStringToFile = string => {
  _fs.default.appendFileSync(_index.fileName, string, error => console.log(error));
}; // Append a new line to file zero or more times, log if error


let appendNewLineToFile = repetitions => {
  for (let i = 0; i <= repetitions; i++) {
    _fs.default.appendFileSync(_index.fileName, _os.default.EOL, error => console.log(error));
  }
};
/**
...
let getAssetNameVariations = (baseAsset, quoteAsset) => {
  let variationString = ''; // Special case if bitcoin (BTC, XBT) is the baseAsset
  ...
  if (baseAsset == 'BTC' || baseAsset == 'XBT') {
    ...
    let baseAssetVariation = baseAsset in _index.tickerVariations ? _index.tickerVariations[baseAsset] : '';
    let quoteAssetVariation = quoteAsset == 'BTC' ? 'XBT' : 'BTC';
    let hasXBTPair = baseAssetsWithXBTPairs.includes(baseAsset) || baseAssetsWithXBTPairs.includes(baseAssetVariation); // Include if the baseAsset has an XBT quoteAsset pair
...
let generatePineScript = pairs => {
  console.log(_chalk.default.white("Generating the Pinescript file..."));

  let pinescriptSkeletonTop = _fs.default.readFileSync('./src/skeleton/pinescript-skeleton-top.txt', {
    encoding: 'utf-8',
    flag: 'r'
  });

  let pinescriptSkeletonBottom = _fs.default.readFileSync('./src/skeleton/pinescript-skeleton-bottom.txt', {
    encoding: 'utf-8',
    flag: 'r'
  });
  ...
  _fs.default.closeSync(_fs.default.openSync(_index.fileName, 'w')); // Creates a write-only file if it doesn't exist, truncates the contents otherwise
  ...
  if (scopeCounter + 2 > 500) {
    console.log(_chalk.default.red('WARNING: The script has ' + (scopeCounter + 2) + ' local scopes and will not compile successfully. Tradingview imposes a 500 local scopes limit per script. Try reducing the number of asset pairs.'));
  }

  console.log(_chalk.default.green("Compiled successfully!"));
};

var _default = generatePineScript;
