...
let parseBitstamp = json => {
  let pairs = {};
  json.forEach(pair => {
    let splitPair = _lodash.default.split(pair.name, '/', 2);
...
let parseKraken = json => {
  ...
  for (let baseObject in results) {
    let splitPair = _lodash.default.split(results[baseObject].wsname, '/', 2);
...
let parsePoloniex = json => {
  ...
  for (let pair in json) {
    let splitPair = _lodash.default.split(pair, '_', 2);
...
let parseResponse = (json, exchangeName) => {
  ...
  switch (exchangeName) {
    case 'binance':
      pairs = parseBinance(json);
      break;

    case 'bitfinex':
      pairs = parseBitfinex(json);
      break;

    case 'bitstamp':
      pairs = parseBitstamp(json);
      break;

    case 'bittrex':
      pairs = parseBittrex(json);
      break;

    case 'coinbase':
      pairs = parseCoinbase(json);
      break;

    case 'gemini':
      pairs = parseGemini(json);
      break;

    case 'kraken':
      pairs = parseKraken(json);
      break;

    case 'poloniex':
      pairs = parsePoloniex(json);
      break;
...
let exchange = (exchangeName, exchangeAPIUrl) => {
  console.log(_chalk.default.white('Fetching ' + _lodash.default.capitalize(exchangeName) + ' pairs...'));
  return new Promise(resolve => {
    (0, _nodeFetch.default)(exchangeAPIUrl).then(response => response.ok ? response.json() : Promise.reject(_lodash.default.capitalize(exchangeName) + ' GET API call failed with status ' + response.status)).then(json => {
      console.log(_chalk.default.white('Parsing response and sorting ' + _lodash.default.capitalize(exchangeName) + ' pairs...'));
      let pairs = parseResponse(json, exchangeName);
      pairs = sortPairsByKey(pairs);
      resolve(pairs);
    }).catch(error => console.log(_chalk.default.red(error)));
  });
};
